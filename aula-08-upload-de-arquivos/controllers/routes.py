...
# IMPORTANDO A BIBLIOTECA OS(MANIPULAM O SISTEMA OPERACIONAL DO USUARIO)
import os
#IMPOSTANDO A BIBLIOTECA UUID(CRIA NOMES ALEATORIOAS PARA OS ARQUIVOS)
import uuid
...
def init_app(app):
    ...
    def apigames(id=None):
        ...
        # VERIFICANDO SE FOR PASSADO UMA ID PARA ROTA   
        if id:
            jogoInfo = []
            ...
            if jogoInfo:
                ...
                return f'Game com ID {id} não encontrado!'
        else:
    ...
    def galeria():
        ...
        # RECEBENDO O ARQUIVO DO USUÁRIO
        if request.method == 'POST':
            #Captura o arquivo do formulario
            file = request.files['file']
            # enviando o arquivos para verificar de a extensão é permitida
            if not arquivos_permitidos(file.filename):
                flash("Arquivo não permitido! Ultileze apenas arquivos de imagens!","danger")
